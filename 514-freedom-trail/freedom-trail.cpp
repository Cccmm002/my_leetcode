...
class Solution {
private:
    inline int steps(int cur, int target, int total) {
        int a = min(cur, target);
        int b = max(cur, target);
        return min(b - a, a + total - b);
    }
...
public:
    int findRotateSteps(string ring, string key) {
        ...
        int keyLen = key.size();
        vector<vector<int>> f(keyLen);
        for (int i = 0; i < keyLen; i++) {
            vector<int> v(ringLen, INT_MAX);
            f[i] = v;
        }
        for (int i = 0; i < ringLen; i++) {
            for (int j = 0; j < ringLen; j++) {
                //if (i == j) continue;
                if (ring[j] != key[keyLen - 1]) continue;
                f[keyLen - 1][i] = min(f[keyLen - 1][i], steps(i, j, ringLen) + 1);
            }
        }
        for (int i = keyLen - 2; i >= 0; i--) {
            for (int k = 0; k < ringLen; k++) {
                for (int kp = 0; kp < ringLen; kp++) {
                    if (ring[kp] != key[i]) continue;
                    f[i][k] = min(f[i][k], f[i + 1][kp] + steps(k, kp, ringLen) + 1);
                }
            }
        }
        return f[0][0];
